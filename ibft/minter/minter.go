...
func (m *Minter) createWork() *work {
	...
	publicState, privateStateRepo, err := m.eth.BlockChain().StateAt(parent.Root())
	if err != nil {
		panic(fmt.Sprint("failed to get parent state: ", err))
	}

	defaultPrivateState, err := privateStateRepo.DefaultState()
	if err != nil {
...
func (m *Minter) RequestBlock() <-chan *types.Block {
	log.Info("IBFT - ->Minter#RequestBlock")
	//m.mu.Lock()
	proposal := make(chan *types.Block)

	go func() {
		log.Info("IBFT: async ->Minter#RequestBlock")
		log.Info("IBFT: async Minter#RequestBlock - waiting for pending txs")
		cbTime := time.Now()

		// This is an ugly hack. Not the way to fix a race condition in the pool.
		...
		log.Info("IBFT: async Minter#RequestBlock - created block", "#", block.Number(), "txs", len(block.Transactions()))
		if err != nil {
			panic(err) // FIXME: proper error handling
		}
		proposal <- block
		log.Info("IBFT: async Minter#RequestBlock->")
		log.Info("IBFT: createBlock time", "block", block.NumberU64(), "time", time.Since(cbTime))
	}()
	log.Info("IBFT - Minter#RequestBlock->")
	return proposal
}
...
func (m *Minter) commitTransaction(
	...
	publicReceipt, privateReceipt, err := core.ApplyTransaction(
		m.eth.BlockChain().Config(), m.eth.BlockChain(), author, gp, work.publicState, work.privateState, work.header, tx, &work.header.GasUsed, vmConf, true, nil)
	if err != nil {
